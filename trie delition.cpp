...
{trie_ptr tmp1,tmp2;tmp2=NULL;
	...
	{int i=0;	
		...
		{if(t->flag==1)
			{
			if(t->info.trptr[pos(s[i])]!=NULL)
			{tmp1=t->info.trptr[pos(s[i])];
				if(tmp1->flag==0)
					{	if(strncmp(tmp1->info.data,s,100)==0)
						{t->info.trptr[pos(s[i])]=NULL;
						cout<<"deleted!!!";
						if(count(t)==1&&tmp2!=NULL)
						tmp2->info.trptr[pos(s[i-1])]=get_tr(t);
						break;
						}
						else
